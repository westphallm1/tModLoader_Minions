...
namespace AmuletOfManyMinions.Core.Minions.CombatPetsQuiz
{
	...
	{
		public List<CombatPetsQuizQuestion> Questions { get; set; } = new List<CombatPetsQuizQuestion>();

		public readonly List<PersonalityType> GivenAnswers = new List<PersonalityType>();

		private int currentQuestionIdx = 0;
		private int dialogIdx = 0;

		internal string[] IntroLines = new string[0];
		internal string[] OutroLines = new string[0];
		...
		internal string CurrentDialogText {
			get {
				switch(CurrentState)
				{
					case QuizState.INTRO: return IntroLines[dialogIdx];
					case QuizState.QUIZ: return CurrentQuestion.QuestionText;
					case QuizState.OUTRO: return string.Format(OutroLines[dialogIdx], Result?.Description ?? "", Result?.PetName ?? "");
					default: return "";
				}
			}
		} 
	...
	internal class QuizResult
	{
		...
		internal Texture2D PortraitTexture { get; set; }
		...
		internal static Dictionary<PersonalityType, QuizResult> MakeResultsMap() => new Dictionary<PersonalityType, QuizResult>()
		{
